...
test('no unchanged Realtime tool definition drifts silently', () => {
  ...
  const TOUCHED = new Set([
    ...
    'set_map_stack',
    // IterativWorld: the World Context Engine adds explain_view as a new
    // tool (Phase 1); the pre-existing tools stay byte-identical.
    'explain_view',
  ]);
  ...
  assert.equal(digest, '6da6aa2a5eab2bee', 'an unchanged Realtime tool definition drifted');
});
